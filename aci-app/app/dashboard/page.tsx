...
export default async function DashboardPage() {
  ...
  const projetsTitre = "Projets en cours";
  const revenusTitre = "Revenus nets";
  const associesTitre = "Associés actifs";
  ...
  try {
    ...
    try {
      ...
      console.log("DashboardPage: Associés récupérés:", associes.length);
      coGerantsCount = associes.filter(associe => associe.est_co_gerant).length;
    } catch (err) {
    ...
    try {
      ...
      projetsActifs = await getProjetsActifs();
      console.log("DashboardPage: Projets actifs récupérés:", projetsActifs.length);
    } catch (err) {
    ...
    try {
      ...
      totalRevenus = await getRevenusTotaux();
      console.log("DashboardPage: Revenus récupérés:", totalRevenus);
    } catch (err) {
    ...
    try {
      ...
      totalCharges = await getChargesTotales();
      console.log("DashboardPage: Charges récupérées:", totalCharges);
    } catch (err) {
    ...
    try {
      ...
      console.log("DashboardPage: Paramètres récupérés:", parametres ? "OK" : "NON");
    } catch (err) {
  ...
  revenuNet = totalRevenus - totalCharges;
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">{revenusTitre}</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">{associesTitre}</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">{projetsTitre}</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <CardContent>
              <div className="space-y-2">
                ...
                <div className="flex justify-between border-t pt-2">
                  <span className="text-sm font-medium">Revenus nets:</span>
                  <span className="text-sm font-medium">{formatMontant(revenuNet)}</span>
                </div>
              </div>
              <div className="mt-4">
                <Link href="/dashboard/revenus" className="text-sm text-primary hover:underline">
                  Gérer les revenus et charges
                </Link>
              </div>
            </CardContent>
          </Card>
